// Code generated - DO NOT EDIT.
// This file is a generated binding and any manual changes will be lost.

package reserva

import (
	"errors"
	"math/big"
	"strings"

	ethereum "github.com/ethereum/go-ethereum"
	"github.com/ethereum/go-ethereum/accounts/abi"
	"github.com/ethereum/go-ethereum/accounts/abi/bind"
	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/event"
)

// Reference imports to suppress errors if they are not otherwise used.
var (
	_ = errors.New
	_ = big.NewInt
	_ = strings.NewReader
	_ = ethereum.NotFound
	_ = bind.Bind
	_ = common.Big1
	_ = types.BloomLookup
	_ = event.NewSubscription
	_ = abi.ConvertType
)

// ReservaMetaData contains all meta data concerning the Reserva contract.
var ReservaMetaData = &bind.MetaData{
	ABI: "[{\"inputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"constructor\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"reservaId\",\"type\":\"uint256\"}],\"name\":\"cancelarReserva\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"contadorReservas\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"usuario\",\"type\":\"address\"}],\"name\":\"getReservasDoUsuario\",\"outputs\":[{\"internalType\":\"uint256[]\",\"name\":\"ids\",\"type\":\"uint256[]\"},{\"internalType\":\"uint256[]\",\"name\":\"pontos\",\"type\":\"uint256[]\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[],\"name\":\"owner\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"pontoId\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"inicio\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"fim\",\"type\":\"uint256\"}],\"name\":\"reservarPonto\",\"outputs\":[],\"stateMutability\":\"nonpayable\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"reservas\",\"outputs\":[{\"internalType\":\"address\",\"name\":\"usuario\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"pontoId\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"inicio\",\"type\":\"uint256\"},{\"internalType\":\"uint256\",\"name\":\"fim\",\"type\":\"uint256\"},{\"internalType\":\"bool\",\"name\":\"ativa\",\"type\":\"bool\"}],\"stateMutability\":\"view\",\"type\":\"function\"},{\"inputs\":[{\"internalType\":\"address\",\"name\":\"\",\"type\":\"address\"},{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"name\":\"reservasPorUsuario\",\"outputs\":[{\"internalType\":\"uint256\",\"name\":\"\",\"type\":\"uint256\"}],\"stateMutability\":\"view\",\"type\":\"function\"}]",
	Bin: "",
}

// ReservaABI is the input ABI used to generate the binding from.
// Deprecated: Use ReservaMetaData.ABI instead.
var ReservaABI = ReservaMetaData.ABI

// ReservaBin is the compiled bytecode used for deploying new contracts.
// Deprecated: Use ReservaMetaData.Bin instead.
var ReservaBin = ReservaMetaData.Bin

// DeployReserva deploys a new Ethereum contract, binding an instance of Reserva to it.
func DeployReserva(auth *bind.TransactOpts, backend bind.ContractBackend) (common.Address, *types.Transaction, *Reserva, error) {
	parsed, err := ReservaMetaData.GetAbi()
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	if parsed == nil {
		return common.Address{}, nil, nil, errors.New("GetABI returned nil")
	}

	address, tx, contract, err := bind.DeployContract(auth, *parsed, common.FromHex(ReservaBin), backend)
	if err != nil {
		return common.Address{}, nil, nil, err
	}
	return address, tx, &Reserva{ReservaCaller: ReservaCaller{contract: contract}, ReservaTransactor: ReservaTransactor{contract: contract}, ReservaFilterer: ReservaFilterer{contract: contract}}, nil
}

// Reserva is an auto generated Go binding around an Ethereum contract.
type Reserva struct {
	ReservaCaller     // Read-only binding to the contract
	ReservaTransactor // Write-only binding to the contract
	ReservaFilterer   // Log filterer for contract events
}

// ReservaCaller is an auto generated read-only Go binding around an Ethereum contract.
type ReservaCaller struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// ReservaTransactor is an auto generated write-only Go binding around an Ethereum contract.
type ReservaTransactor struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// ReservaFilterer is an auto generated log filtering Go binding around an Ethereum contract events.
type ReservaFilterer struct {
	contract *bind.BoundContract // Generic contract wrapper for the low level calls
}

// ReservaSession is an auto generated Go binding around an Ethereum contract,
// with pre-set call and transact options.
type ReservaSession struct {
	Contract     *Reserva          // Generic contract binding to set the session for
	CallOpts     bind.CallOpts     // Call options to use throughout this session
	TransactOpts bind.TransactOpts // Transaction auth options to use throughout this session
}

// ReservaCallerSession is an auto generated read-only Go binding around an Ethereum contract,
// with pre-set call options.
type ReservaCallerSession struct {
	Contract *ReservaCaller // Generic contract caller binding to set the session for
	CallOpts bind.CallOpts  // Call options to use throughout this session
}

// ReservaTransactorSession is an auto generated write-only Go binding around an Ethereum contract,
// with pre-set transact options.
type ReservaTransactorSession struct {
	Contract     *ReservaTransactor // Generic contract transactor binding to set the session for
	TransactOpts bind.TransactOpts  // Transaction auth options to use throughout this session
}

// ReservaRaw is an auto generated low-level Go binding around an Ethereum contract.
type ReservaRaw struct {
	Contract *Reserva // Generic contract binding to access the raw methods on
}

// ReservaCallerRaw is an auto generated low-level read-only Go binding around an Ethereum contract.
type ReservaCallerRaw struct {
	Contract *ReservaCaller // Generic read-only contract binding to access the raw methods on
}

// ReservaTransactorRaw is an auto generated low-level write-only Go binding around an Ethereum contract.
type ReservaTransactorRaw struct {
	Contract *ReservaTransactor // Generic write-only contract binding to access the raw methods on
}

// NewReserva creates a new instance of Reserva, bound to a specific deployed contract.
func NewReserva(address common.Address, backend bind.ContractBackend) (*Reserva, error) {
	contract, err := bindReserva(address, backend, backend, backend)
	if err != nil {
		return nil, err
	}
	return &Reserva{ReservaCaller: ReservaCaller{contract: contract}, ReservaTransactor: ReservaTransactor{contract: contract}, ReservaFilterer: ReservaFilterer{contract: contract}}, nil
}

// NewReservaCaller creates a new read-only instance of Reserva, bound to a specific deployed contract.
func NewReservaCaller(address common.Address, caller bind.ContractCaller) (*ReservaCaller, error) {
	contract, err := bindReserva(address, caller, nil, nil)
	if err != nil {
		return nil, err
	}
	return &ReservaCaller{contract: contract}, nil
}

// NewReservaTransactor creates a new write-only instance of Reserva, bound to a specific deployed contract.
func NewReservaTransactor(address common.Address, transactor bind.ContractTransactor) (*ReservaTransactor, error) {
	contract, err := bindReserva(address, nil, transactor, nil)
	if err != nil {
		return nil, err
	}
	return &ReservaTransactor{contract: contract}, nil
}

// NewReservaFilterer creates a new log filterer instance of Reserva, bound to a specific deployed contract.
func NewReservaFilterer(address common.Address, filterer bind.ContractFilterer) (*ReservaFilterer, error) {
	contract, err := bindReserva(address, nil, nil, filterer)
	if err != nil {
		return nil, err
	}
	return &ReservaFilterer{contract: contract}, nil
}

// bindReserva binds a generic wrapper to an already deployed contract.
func bindReserva(address common.Address, caller bind.ContractCaller, transactor bind.ContractTransactor, filterer bind.ContractFilterer) (*bind.BoundContract, error) {
	parsed, err := ReservaMetaData.GetAbi()
	if err != nil {
		return nil, err
	}
	return bind.NewBoundContract(address, *parsed, caller, transactor, filterer), nil
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_Reserva *ReservaRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _Reserva.Contract.ReservaCaller.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_Reserva *ReservaRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Reserva.Contract.ReservaTransactor.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_Reserva *ReservaRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _Reserva.Contract.ReservaTransactor.contract.Transact(opts, method, params...)
}

// Call invokes the (constant) contract method with params as input values and
// sets the output to result. The result type might be a single field for simple
// returns, a slice of interfaces for anonymous returns and a struct for named
// returns.
func (_Reserva *ReservaCallerRaw) Call(opts *bind.CallOpts, result *[]interface{}, method string, params ...interface{}) error {
	return _Reserva.Contract.contract.Call(opts, result, method, params...)
}

// Transfer initiates a plain transaction to move funds to the contract, calling
// its default method if one is available.
func (_Reserva *ReservaTransactorRaw) Transfer(opts *bind.TransactOpts) (*types.Transaction, error) {
	return _Reserva.Contract.contract.Transfer(opts)
}

// Transact invokes the (paid) contract method with params as input values.
func (_Reserva *ReservaTransactorRaw) Transact(opts *bind.TransactOpts, method string, params ...interface{}) (*types.Transaction, error) {
	return _Reserva.Contract.contract.Transact(opts, method, params...)
}

// ContadorReservas is a free data retrieval call binding the contract method 0xaa34dd42.
//
// Solidity: function contadorReservas() view returns(uint256)
func (_Reserva *ReservaCaller) ContadorReservas(opts *bind.CallOpts) (*big.Int, error) {
	var out []interface{}
	err := _Reserva.contract.Call(opts, &out, "contadorReservas")

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// ContadorReservas is a free data retrieval call binding the contract method 0xaa34dd42.
//
// Solidity: function contadorReservas() view returns(uint256)
func (_Reserva *ReservaSession) ContadorReservas() (*big.Int, error) {
	return _Reserva.Contract.ContadorReservas(&_Reserva.CallOpts)
}

// ContadorReservas is a free data retrieval call binding the contract method 0xaa34dd42.
//
// Solidity: function contadorReservas() view returns(uint256)
func (_Reserva *ReservaCallerSession) ContadorReservas() (*big.Int, error) {
	return _Reserva.Contract.ContadorReservas(&_Reserva.CallOpts)
}

// GetReservasDoUsuario is a free data retrieval call binding the contract method 0xad852310.
//
// Solidity: function getReservasDoUsuario(address usuario) view returns(uint256[] ids, uint256[] pontos)
func (_Reserva *ReservaCaller) GetReservasDoUsuario(opts *bind.CallOpts, usuario common.Address) (struct {
	Ids    []*big.Int
	Pontos []*big.Int
}, error) {
	var out []interface{}
	err := _Reserva.contract.Call(opts, &out, "getReservasDoUsuario", usuario)

	outstruct := new(struct {
		Ids    []*big.Int
		Pontos []*big.Int
	})
	if err != nil {
		return *outstruct, err
	}

	outstruct.Ids = *abi.ConvertType(out[0], new([]*big.Int)).(*[]*big.Int)
	outstruct.Pontos = *abi.ConvertType(out[1], new([]*big.Int)).(*[]*big.Int)

	return *outstruct, err

}

// GetReservasDoUsuario is a free data retrieval call binding the contract method 0xad852310.
//
// Solidity: function getReservasDoUsuario(address usuario) view returns(uint256[] ids, uint256[] pontos)
func (_Reserva *ReservaSession) GetReservasDoUsuario(usuario common.Address) (struct {
	Ids    []*big.Int
	Pontos []*big.Int
}, error) {
	return _Reserva.Contract.GetReservasDoUsuario(&_Reserva.CallOpts, usuario)
}

// GetReservasDoUsuario is a free data retrieval call binding the contract method 0xad852310.
//
// Solidity: function getReservasDoUsuario(address usuario) view returns(uint256[] ids, uint256[] pontos)
func (_Reserva *ReservaCallerSession) GetReservasDoUsuario(usuario common.Address) (struct {
	Ids    []*big.Int
	Pontos []*big.Int
}, error) {
	return _Reserva.Contract.GetReservasDoUsuario(&_Reserva.CallOpts, usuario)
}

// Owner is a free data retrieval call binding the contract method 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (_Reserva *ReservaCaller) Owner(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _Reserva.contract.Call(opts, &out, "owner")

	if err != nil {
		return *new(common.Address), err
	}

	out0 := *abi.ConvertType(out[0], new(common.Address)).(*common.Address)

	return out0, err

}

// Owner is a free data retrieval call binding the contract method 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (_Reserva *ReservaSession) Owner() (common.Address, error) {
	return _Reserva.Contract.Owner(&_Reserva.CallOpts)
}

// Owner is a free data retrieval call binding the contract method 0x8da5cb5b.
//
// Solidity: function owner() view returns(address)
func (_Reserva *ReservaCallerSession) Owner() (common.Address, error) {
	return _Reserva.Contract.Owner(&_Reserva.CallOpts)
}

// Reservas is a free data retrieval call binding the contract method 0x9f436853.
//
// Solidity: function reservas(uint256 ) view returns(address usuario, uint256 pontoId, uint256 inicio, uint256 fim, bool ativa)
func (_Reserva *ReservaCaller) Reservas(opts *bind.CallOpts, arg0 *big.Int) (struct {
	Usuario common.Address
	PontoId *big.Int
	Inicio  *big.Int
	Fim     *big.Int
	Ativa   bool
}, error) {
	var out []interface{}
	err := _Reserva.contract.Call(opts, &out, "reservas", arg0)

	outstruct := new(struct {
		Usuario common.Address
		PontoId *big.Int
		Inicio  *big.Int
		Fim     *big.Int
		Ativa   bool
	})
	if err != nil {
		return *outstruct, err
	}

	outstruct.Usuario = *abi.ConvertType(out[0], new(common.Address)).(*common.Address)
	outstruct.PontoId = *abi.ConvertType(out[1], new(*big.Int)).(**big.Int)
	outstruct.Inicio = *abi.ConvertType(out[2], new(*big.Int)).(**big.Int)
	outstruct.Fim = *abi.ConvertType(out[3], new(*big.Int)).(**big.Int)
	outstruct.Ativa = *abi.ConvertType(out[4], new(bool)).(*bool)

	return *outstruct, err

}

// Reservas is a free data retrieval call binding the contract method 0x9f436853.
//
// Solidity: function reservas(uint256 ) view returns(address usuario, uint256 pontoId, uint256 inicio, uint256 fim, bool ativa)
func (_Reserva *ReservaSession) Reservas(arg0 *big.Int) (struct {
	Usuario common.Address
	PontoId *big.Int
	Inicio  *big.Int
	Fim     *big.Int
	Ativa   bool
}, error) {
	return _Reserva.Contract.Reservas(&_Reserva.CallOpts, arg0)
}

// Reservas is a free data retrieval call binding the contract method 0x9f436853.
//
// Solidity: function reservas(uint256 ) view returns(address usuario, uint256 pontoId, uint256 inicio, uint256 fim, bool ativa)
func (_Reserva *ReservaCallerSession) Reservas(arg0 *big.Int) (struct {
	Usuario common.Address
	PontoId *big.Int
	Inicio  *big.Int
	Fim     *big.Int
	Ativa   bool
}, error) {
	return _Reserva.Contract.Reservas(&_Reserva.CallOpts, arg0)
}

// ReservasPorUsuario is a free data retrieval call binding the contract method 0x9c33e0af.
//
// Solidity: function reservasPorUsuario(address , uint256 ) view returns(uint256)
func (_Reserva *ReservaCaller) ReservasPorUsuario(opts *bind.CallOpts, arg0 common.Address, arg1 *big.Int) (*big.Int, error) {
	var out []interface{}
	err := _Reserva.contract.Call(opts, &out, "reservasPorUsuario", arg0, arg1)

	if err != nil {
		return *new(*big.Int), err
	}

	out0 := *abi.ConvertType(out[0], new(*big.Int)).(**big.Int)

	return out0, err

}

// ReservasPorUsuario is a free data retrieval call binding the contract method 0x9c33e0af.
//
// Solidity: function reservasPorUsuario(address , uint256 ) view returns(uint256)
func (_Reserva *ReservaSession) ReservasPorUsuario(arg0 common.Address, arg1 *big.Int) (*big.Int, error) {
	return _Reserva.Contract.ReservasPorUsuario(&_Reserva.CallOpts, arg0, arg1)
}

// ReservasPorUsuario is a free data retrieval call binding the contract method 0x9c33e0af.
//
// Solidity: function reservasPorUsuario(address , uint256 ) view returns(uint256)
func (_Reserva *ReservaCallerSession) ReservasPorUsuario(arg0 common.Address, arg1 *big.Int) (*big.Int, error) {
	return _Reserva.Contract.ReservasPorUsuario(&_Reserva.CallOpts, arg0, arg1)
}

// CancelarReserva is a paid mutator transaction binding the contract method 0x2a05ba70.
//
// Solidity: function cancelarReserva(uint256 reservaId) returns()
func (_Reserva *ReservaTransactor) CancelarReserva(opts *bind.TransactOpts, reservaId *big.Int) (*types.Transaction, error) {
	return _Reserva.contract.Transact(opts, "cancelarReserva", reservaId)
}

// CancelarReserva is a paid mutator transaction binding the contract method 0x2a05ba70.
//
// Solidity: function cancelarReserva(uint256 reservaId) returns()
func (_Reserva *ReservaSession) CancelarReserva(reservaId *big.Int) (*types.Transaction, error) {
	return _Reserva.Contract.CancelarReserva(&_Reserva.TransactOpts, reservaId)
}

// CancelarReserva is a paid mutator transaction binding the contract method 0x2a05ba70.
//
// Solidity: function cancelarReserva(uint256 reservaId) returns()
func (_Reserva *ReservaTransactorSession) CancelarReserva(reservaId *big.Int) (*types.Transaction, error) {
	return _Reserva.Contract.CancelarReserva(&_Reserva.TransactOpts, reservaId)
}

// ReservarPonto is a paid mutator transaction binding the contract method 0x7b4ae18c.
//
// Solidity: function reservarPonto(uint256 pontoId, uint256 inicio, uint256 fim) returns()
func (_Reserva *ReservaTransactor) ReservarPonto(opts *bind.TransactOpts, pontoId *big.Int, inicio *big.Int, fim *big.Int) (*types.Transaction, error) {
	return _Reserva.contract.Transact(opts, "reservarPonto", pontoId, inicio, fim)
}

// ReservarPonto is a paid mutator transaction binding the contract method 0x7b4ae18c.
//
// Solidity: function reservarPonto(uint256 pontoId, uint256 inicio, uint256 fim) returns()
func (_Reserva *ReservaSession) ReservarPonto(pontoId *big.Int, inicio *big.Int, fim *big.Int) (*types.Transaction, error) {
	return _Reserva.Contract.ReservarPonto(&_Reserva.TransactOpts, pontoId, inicio, fim)
}

// ReservarPonto is a paid mutator transaction binding the contract method 0x7b4ae18c.
//
// Solidity: function reservarPonto(uint256 pontoId, uint256 inicio, uint256 fim) returns()
func (_Reserva *ReservaTransactorSession) ReservarPonto(pontoId *big.Int, inicio *big.Int, fim *big.Int) (*types.Transaction, error) {
	return _Reserva.Contract.ReservarPonto(&_Reserva.TransactOpts, pontoId, inicio, fim)
}
